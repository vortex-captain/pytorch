...
#    effect is reflected onto the runtime model / inputs). Still UNSUPPORTED and
#    rejected up front (_assert_supported): effectful ops (their custom ops make the
#    Inductor artifact non-saveable). Anything the composition cannot reproduce (e.g.
#    the runtime synthetic-base wrapper for storage-aliased mutated inputs) is caught
#    by a correctness self-check that raises rather than emit a silently-wrong
#    artifact.
#
...
def _assert_supported(gm: torch.fx.GraphModule, flat_args: list[Any]) -> None:
    """Enforce invariant 4 of Note [precompile programming model]: reject the
    boundary effects the AOT backend's standalone composition does not handle.
    Detected directly from the captured graph -- no AOTAutograd coupling.
    ...
    Effectful ops are still rejected up front: their custom ops make the Inductor
    artifact non-saveable, so the inner code cannot be lowered to standalone source.
    """
    ...
    for node in gm.graph.nodes:
        ...
        if node.op == "call_function" and isinstance(
            ...
            if _get_effect(node.target) is not None:
                raise _unsupported("the graph has effectful ops")


def _unsupported(reason: str) -> PrecompileError:
    return PrecompileError(
        f"precompile does not support this computation ({reason}). Input mutation, "
        "tensor subclasses, output aliasing, and functionalized RNG are supported; "
        "effectful ops are not."
    )
